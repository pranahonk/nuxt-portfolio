...
There are **two parallel read paths** for posts:

1. **Development / SSR build**: `server/api/posts/index.ts` and `server/api/posts/[slug].ts` import directly from `server/data/articlesData.ts` (a large in-memory TypeScript array).
2. **Netlify static hosting**: `public/_redirects` maps `/api/posts` → `/api/posts.json` and `/api/posts/:slug` → `/api/posts/:slug.json`. These static JSON files are generated by `scripts/generate-posts-json.js` from the same `server/data/articles/*.json` files.
